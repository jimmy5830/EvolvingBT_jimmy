...
{
    /* 1 SECOND real time is 1*TimeScale MIN
#include "NodeMapComponent.h"UTES game time */
    TimeScale = 8.0f;
    bIsNight = false;
...
void ASGameState::SetTimeOfDay( float NewTimeOfDay ) { ElapsedGameMinutes = NewTimeOfDay; }

bool ASGameState::GetIsNight() { return bIsNight; }

float ASGameState::GetTimeOfDayIncrement() { return ( GetWorldSettings()->GetEffectiveTimeDilation() * TimeScale ); }
...
int32 ASGameState::GetElapsedMinutesCurrentDay() { return ElapsedGameMinutes - GetElapsedFullDaysInMinutes(); }
...
int32 ASGameState::GetTotalScore() { return TotalScore; }

void ASGameState::AddScore( int32 Score ) { TotalScore += Score; }
