    ...
    : Super( ObjectInitializer )
{
    ...
    CarriedObjectComp
        = ObjectInitializer.CreateDefaultSubobject<USCarryObjectComponent>( this, TEXT( "CarriedObjectComp" ) );
...
void ASCharacter::BeginPlay()
{
    ...
    if(HasAuthority() )
	{
	    ...
	    GetWorldTimerManager().SetTimer(
	        Handle, this, &ASCharacter::IncrementHunger, IncrementHungerInterval, true );
	}
}
...
void ASCharacter::ServerUse_Implementation() { Use(); }

bool ASCharacter::ServerUse_Validate() { return true; }
...
void ASCharacter::OnJump() { SetIsJumping( true ); }

bool ASCharacter::IsInitiatedJump() const { return bIsJumping; }
...
bool ASCharacter::ServerSetIsJumping_Validate( bool NewJumping ) { return true; }
...
void ASCharacter::GetLifetimeReplicatedProps( TArray<FLifetimeProperty>& OutLifetimeProps ) const
{
    ...
    /* If we did not display the current inventory on the player mesh we could optimize replication by using this
...
float ASCharacter::GetHunger() const { return Hunger; }

float ASCharacter::GetMaxHunger() const { return MaxHunger; }
...
void ASCharacter::OnDeath( float KillingDamage,
...
bool ASCharacter::CanReload() const { return IsAlive(); }

bool ASCharacter::IsFiring() const { return CurrentWeapon && CurrentWeapon->GetCurrentState() == EWeaponState::Firing; }
...
void ASCharacter::SetCurrentWeapon( class ASWeapon* NewWeapon, class ASWeapon* LastWeapon )
{
    ...
    /* NOTE: If you don't have an equip animation w/ animnotify to swap the meshes halfway through, then uncomment this
     * to immediately swap instead */
    // SwapToNewWeaponMesh();
}

void ASCharacter::OnRep_CurrentWeapon( ASWeapon* LastWeapon ) { SetCurrentWeapon( CurrentWeapon, LastWeapon ); }

ASWeapon* ASCharacter::GetCurrentWeapon() const { return CurrentWeapon; }
...
bool ASCharacter::ServerEquipWeapon_Validate( ASWeapon* Weapon ) { return true; }
...
void ASCharacter::DropWeapon()
{
    ...
    if( CurrentWeapon )
	{
	    ...
	    /* Find a location to drop the item, slightly in front of the player.
	            Perform ray trace to check for blocking objects or walls and to make sure we don't drop any item
	       through the level mesh */
	    Controller->GetPlayerViewPoint( CamLoc, CamRot );
	    ...
	    ASWeaponPickup* NewWeaponPickup = GetWorld()->SpawnActor<ASWeaponPickup>(
...
void ASCharacter::ServerDropWeapon_Implementation() { DropWeapon(); }

bool ASCharacter::ServerDropWeapon_Validate() { return true; }
...
bool ASCharacter::WeaponSlotAvailable( EInventorySlot CheckSlot )
{
    ...
    /* Special find function as alternative to looping the array and performing if statements
            the [=] prefix means "capture by value", other options include [] "capture nothing" and [&] "capture by
       reference" */
...
float ASCharacter::GetLastNoiseLoudness() { return LastNoiseLoudness; }

float ASCharacter::GetLastMakeNoiseTime() { return LastMakeNoiseTime; }

void ASCharacter::Suicide() { KilledBy( this ); }
...
void ASCharacter::OnToggleCarryActor() { CarriedObjectComp->Pickup(); }
